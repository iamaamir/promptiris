...
import { createSupportBundle, MAX_BUNDLE_BYTES } from './support-bundle.js';
...
describe('coverage branches', () => {
  ...
  it('handles consoleSink false', async () => {
  ...
  it('JsonLinesSink capacity overflow and clear', () => {
    ...
    const e = { schemaVersion: '1', id: '1', type: 't', time: new Date().toISOString(), sequence: 0, runId: 'r', traceId: 'r', source: 's', dataSchema: 'd', data: {}, classification: 'metadata', delivery: 'critical' } as Event;
    sink.write(e);
    sink.write(e);
    sink.write(e);
  ...
  it('console sink writer throws isolated', () => {
    const sink = createConsoleSink({ writer: () => { throw new Error('boom'); } });
    const e = { schemaVersion: '1', id: '1', type: 't', time: new Date().toISOString(), sequence: 0, runId: 'r', traceId: 'r', source: 's', dataSchema: 'd', data: {}, classification: 'metadata', delivery: 'critical' } as Event;
    expect(() => sink.write(e)).not.toThrow();
  });
  it('redacts prompt/content/secret in JsonLinesSink', () => {
    const sink = new JsonLinesSink({ capacity: 5 });
    const e = { schemaVersion: '1', id: '1', type: 't', time: new Date().toISOString(), sequence: 0, runId: 'r', traceId: 'r', source: 's', dataSchema: 'd', data: { prompt: 'secret', secret: 's', other: 'keep' }, classification: 'metadata', delivery: 'critical' } as Event;
    sink.write(e);
  ...
  it('sensitive classification redacted', () => {
    const sink = new JsonLinesSink({ capacity: 5 });
    const e = { schemaVersion: '1', id: '1', type: 't', time: new Date().toISOString(), sequence: 0, runId: 'r', traceId: 'r', source: 's', dataSchema: 'd', data: { prompt: 'secret' }, classification: 'sensitive', delivery: 'critical' } as Event;
    sink.write(e);
    expect(sink.lines[0]).toContain('redacted');
  });
  it('support bundle truncates when over MAX_BUNDLE_BYTES', () => {
    ...
    const events: Event[] = Array.from({ length: 60 }, (_, i) => ({
      schemaVersion: '1', id: `id-${i}`, type: 't', time: new Date().toISOString(), sequence: i, runId: 'r', traceId: 'r', source: 's', dataSchema: 'd', data: { largeData }, classification: 'metadata', delivery: i % 2 === 0 ? 'progress' : 'critical',
    })) as Event[];
  ...
  it('handles non-object data', () => {
    const sink = new JsonLinesSink({ capacity: 5 });
    const e = { schemaVersion: '1', id: '1', type: 't', time: new Date().toISOString(), sequence: 0, runId: 'r', traceId: 'r', source: 's', dataSchema: 'd', data: 'string-data' as unknown, classification: 'metadata', delivery: 'critical' } as Event;
    sink.write(e);
  ...
  it('observer getDebugRecords bounded', () => {
    ...
    for (let i = 0; i < 200; i++) d.capture({ id: `${i}`, runId: 'r', traceId: 'r', operation: 'op', exception: { type: 'Error', message: 'm' } } as any);
  ...
  it('observer getEvents bounded', async () => {
    ...
    const d = createObserverDevtools({ observerId: 'test/bound2', maxEvents: 3, consoleSink: false });
    const h = d.attach(disp);
    for (let i = 0; i < 5; i++) disp.emit({ type: `t-${i}`, source: 's', dataSchema: 'd', data: {}, classification: 'metadata', delivery: 'critical' });
