...
Reviewer, Hardener, and source-blind QA reports live beside the Work Item in its evidence directory and conform to portable schemas. A candidate-finalization step freezes the candidate diff and writes its base/digest identity outside Git. Role stages refuse to begin unless that manifest remains current. Each role first creates its factual report, then the deterministic binder adds the frozen identity once; it refuses to relabel a previously bound report. Role-evidence commits are excluded from the candidate digest, avoiding self-referential commit hashes. Implementation, policy, or planning changes invalidate the frozen candidate and require a new cycle. Critical verifier files are CODEOWNED; branch protection must require code-owner review for that ownership declaration to become an enforceable remote merge gate.
