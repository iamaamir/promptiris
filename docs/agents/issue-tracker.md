...
Implementation may contain many commits. Before the Reviewer stage, commit the implementation and Work Item state, then run `pnpm candidate:finalize -- <packet>`. This writes an ignored local manifest with the frozen candidate digest. `agent-work stage reviewer`, `hardener`, `qa`, and `integration` refuse to proceed unless that manifest still matches. A role writes its factual report first, with identity fields absent, then uses `node scripts/bind-role-evidence.mjs <role>` to bind it once. A previously bound report is never relabeled, even for the same candidate. Evidence commits do not invalidate the frozen candidate; implementation, policy, or planning changes require a new cycle.
